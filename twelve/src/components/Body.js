...
const Body = () => {  //passing user state in body component from app.js by using destructuring - removed later 
    ...
    const {user,setUser} = useContext(UserContext)           //extracting value of UserContext , user and setUser from app.js
    ...
    (
       <>
       <div className="p-5 bg-pink-100 m-2">
        <input type="text" 
        ...
        <input  className="p-2 m-4"
        value={user.name}   //using user.name 
        onChange = {e =>         //setuser to user object 
          setUser({
           ...user,                       //user for name to modify name 
            name: e.target.value,
          })    
        }
        >
        ...
        value={user.email}   //using user.name 
        onChange = {e =>         //setuser to user object 
          setUser({
            ...user,                   //using email to modify email id 
      ...
      {/*write logic for no restaurants found */}
      {
        filteredRestaurants.map((restaurant) => { //using map to render the restuarants without typing again and again 
          ...
          <RestaurantCard {...restaurant.data}
            // user = {user}   //passing user state variablle as props from body component 
          /></Link> 
             );
        })
      }
